...
def send_message_to_2_telegram_users(first_profile_id, second_profile_id, text1, text2):
    ...
    username = requests.request("POST", url, headers=headers, data=payload).json().get("result").get("user").get(
        "username")
